...
export function SignUpForm() {
	...
	const validateForm = (formData: FormData) => {
		...
		if (!email || !email.includes("@")) {
			errors.email = t.auth.signUp.errors.invalidEmail;
		}
		...
		try {
			...
			if (!response.ok) {
				if (result.error === "EMAIL_EXISTS") {
					setErrors({ email: t.auth.signUp.errors.emailExists });
				} else {
					setErrors({ general: result.error || t.auth.signUp.errors.registrationFailed });
				}
			} else {
				setEmailSent(true);
			}
	...
	if (emailSent) {
		return (
			<div className="bg-card/50 backdrop-blur-sm rounded-2xl border p-8 shadow-xl text-center">
				...
				<h2 className="text-2xl font-bold mb-4">{t.auth.signUp.emailSent.title}</h2>
				<p className="text-muted-foreground mb-6">
					{t.auth.signUp.emailSent.message}
				</p>
				<div className="text-sm text-muted-foreground">
					{t.auth.signUp.emailSent.didNotReceive}{" "}
					<button className="text-primary hover:underline font-medium">
						{t.auth.signUp.emailSent.resendVerification}
					</button>
				</div>
			</div>
		);
		...
		<div className="bg-card/50 backdrop-blur-sm rounded-2xl border p-8 shadow-xl">
			...
			<div className="relative my-6">
				...
				<div className="relative flex justify-center text-sm">
					<span className="bg-card px-4 text-muted-foreground">{t.auth.signUp.or}</span>
				</div>
			</div>
